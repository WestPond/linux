...
static int cxusb_mxl692_frontend_attach(struct dvb_usb_adapter *adap)
{
	struct dvb_usb_device *d = adap->dev;
	struct cxusb_state *st = d->priv;
	...
	mxl692_config.id = 0;
	mxl692_config.fe = &adap->fe_adap[0].fe;

	st->i2c_client_demod = dvb_module_probe("mxl692", NULL,
						&adap->dev->i2c_adap,
						0x60, &mxl692_config);
...
static struct dvb_usb_device_properties cxusb_ma692_properties;
		       ...
		       const struct usb_device_id *id)
{
	if (0 == dvb_usb_device_init(intf, &cxusb_medion_properties,
	    ...
	    0 == dvb_usb_device_init(intf, &cxusb_mygica_t230_properties,
				     THIS_MODULE, NULL, adapter_nr) ||
	    0 == dvb_usb_device_init(intf, &cxusb_ma692_properties,
				     THIS_MODULE, NULL, adapter_nr) ||
	    0)
		return 0;
...
static struct usb_device_id cxusb_table[NR__cxusb_table_index + 1] = {
	...
	[GENIATECH_MA692] = {
		USB_DEVICE(USB_VID_GTEK, USB_PID_MA692_TUNER)
	},
	{}		/* Terminating entry */
};
...
static struct dvb_usb_device_properties cxusb_ma692_properties = {
	...
	.adapter = {
		...
		.fe = {{
			.streaming_ctrl   = cxusb_d680_dmb_streaming_ctrl,
			.frontend_attach  = cxusb_mxl692_frontend_attach,
	...
	.devices = {
		{
			"Geniatech MA692 ATSC/QAM Tuner",
			{ NULL },
			{ &cxusb_table[GENIATECH_MA692], NULL },
		},
	}
};
