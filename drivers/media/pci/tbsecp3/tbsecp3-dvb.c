...
#ifdef CONFIG_DVB_MXL58X
#include "mxl58x.h"
#endif

#ifdef CONFIG_DVB_SI2183
#include "si2183.h"
#endif
...
#if defined(CONFIG_DVB_MXL58X) || defined(CONFIG_DVB_STID135)
...
static int tbsecp3_frontend_attach(struct tbsecp3_adapter *adapter)
{
	...
	case TBSECP3_BOARD_TBS6590:
		...
		si2183_config.i2c_adapter = &i2c;
		si2183_config.fe = &adapter->fe;
		si2183_config.ts_mode = dev->pci_dev->subsystem_vendor==0x6528 ? SI2183_TS_PARALLEL : SI2183_TS_SERIAL;
		si2183_config.ts_clock_gapped = true;
		si2183_config.rf_in = adapter->nr;
		...
		memset(&info, 0, sizeof(struct i2c_board_info));
		strlcpy(info.type, "si2183", I2C_NAME_SIZE);
		if(dev->pci_dev->subsystem_vendor==0x6528)
		{
		    info.addr = 0x67;
		...
		memset(&info, 0, sizeof(struct i2c_board_info));
		strlcpy(info.type, "si2157", I2C_NAME_SIZE);
		if(dev->pci_dev->subsystem_vendor==0x6528)info.addr = 0x61;
		else
	...
	case TBSECP3_BOARD_TBS6912:
		if(dev->pci_dev->subsystem_vendor==0x6912)
			adapter->fe = dvb_attach(stid135_attach, i2c,
