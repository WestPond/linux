				 ...
				 u32 buf_len, u32 *index)
{
	...
	if (status)
		dev_err(&dev->i2c_client->dev, "mxl692_write_fw_block err %d\n", status);
			   ...
			   u8 *buffer, u32 size)
{
	...
	*(u32 *)plocal_buf = addr;
	...
	return status;
err_finish:
	dev_err(&dev->i2c_client->dev, "mxl692_memwrite err %d\n", status);
	return status;
}
			  ...
			  u8 *buffer, u32 size)
{
	...
	*(u32 *)plocal_buf = addr;
	...
	if (status)
		dev_err(&dev->i2c_client->dev, "mxl692_memread err %d\n", status);
...
err_finish:
	if (status)
		dev_err(&dev->i2c_client->dev, "mxl692_opwrite err %d\n", status);
	return status;
}
...
err_finish:
	if (status)
		dev_err(&dev->i2c_client->dev, "mxl692_opread err %d\n", status);
	return status;
}
...
err_finish:
	if (status)
		dev_err(&dev->i2c_client->dev, "mxl692_i2c_writeread err %d\n", status);

	mutex_unlock(&dev->i2c_lock);
	return status;
}

static int mxl692_get_status(struct mxl692_dev *dev,
				 struct MXL_EAGLE_DEV_STATUS_T *dev_status)
{
	...
	if (status)
		dev_err(&dev->i2c_client->dev, "mxl692_get_status err %d\n", status);
			     ...
			     const u8 *firmware_buf, u32 buf_len)
{
	...
	dev_dbg(&dev->i2c_client->dev, "mxl692_fwdownload: %u bytes\n", buf_len);
	...
	dev_dbg(&dev->i2c_client->dev, "mxl692_fwdownload loaded\n");
	...
	dev_dbg(&dev->i2c_client->dev, "mxl692_fwdownload CPU reset\n");

	mutex_unlock(&dev->i2c_lock);

	/* verify FW is alive */
	usleep_range(MXL_EAGLE_FW_LOAD_TIME * 1000, (MXL_EAGLE_FW_LOAD_TIME + 10) * 1000);
	return mxl692_get_status(dev, (struct MXL_EAGLE_DEV_STATUS_T *)&rx_buf);

err_finish:
	mutex_unlock(&dev->i2c_lock);
	if (status)
		dev_err(&dev->i2c_client->dev, "mxl692_fwdownload err %d\n", status);
	return status;
}
...
static int mxl692_reset(struct mxl692_dev *dev)
{
	...
	dev_dbg(&dev->i2c_client->dev, "mxl692_reset\n");
	...
	/* verify sku */
	status = mxl692_memread(dev, 0x9004000c, (u8 *)&reg_val, sizeof(u32));
	if (status)
		goto err_finish;
	...
	status = mxl692_memwrite(dev, 0x9004000c, (u8 *)&reg_val, sizeof(u32));
	if (status)
		goto err_finish;
	...
	if (status)
		goto err_finish;
	dev_dbg(&dev->i2c_client->dev, "mxl692_reset dev_type %u\n", dev_type);
...
err_finish:
	if (status)
		dev_err(&dev->i2c_client->dev, "mxl692_reset err %d\n", status);
	return status;
}
				    ...
				    enum MXL_EAGLE_POWER_SUPPLY_SOURCE_E power_supply)
{
	...
	dev_dbg(&dev->i2c_client->dev, "mxl692_config_regulators\n");
...
err_finish:
	if (status)
		dev_err(&dev->i2c_client->dev, "mxl692_config_regulators err %d\n", status);
	return status;
}
			      ...
			      struct MXL_EAGLE_DEV_XTAL_T *dev_xtal)
{
	...
	dev_dbg(&dev->i2c_client->dev, "mxl692_config_xtal\n");

	status = mxl692_i2c_writeread(dev,
				      MXL_EAGLE_OPCODE_DEVICE_XTAL_CALIBRATION_SET,
				      (u8 *)dev_xtal,
				      sizeof(struct MXL_EAGLE_DEV_XTAL_T),
				      NULL,
				      0);

	if (status)
		dev_err(&dev->i2c_client->dev, "mxl692_config_xtal err %d\n", status);
			    ...
			    enum MXL_EAGLE_POWER_MODE_E power_mode)
{
	...
	status = mxl692_i2c_writeread(dev,
				      ...
				      0);
	if (status)
		dev_err(&dev->i2c_client->dev, "mxl692_powermode err %d\n", status);
...
static int mxl692_set_demod(struct mxl692_dev *dev,
			    enum MXL_EAGLE_DEMOD_TYPE_E demod_type)
{
	int status = 0;
	u8 val = demod_type;
	...
	status = mxl692_i2c_writeread(dev,
				      ...
				      0);
	if (status)
		dev_err(&dev->i2c_client->dev, "mxl692_set_demod err %d\n", status);
...
static int mxl692_init(struct dvb_frontend *fe)
{
	...
	dev_dbg(&dev->i2c_client->dev, "mxl692_init\n");
	...
	if (status) {
		dev_err(&dev->i2c_client->dev, "firmware missing? %s\n",
			MXL692_FIRMWARE);
		goto err;
	}
	...
	xtal_config.xtal_calibration_enable = 1;
	xtal_config.xtal_sharing_enable = 0;
...
static int mxl692_set_frontend(struct dvb_frontend *fe)
{
	...
	dev_dbg(&dev->i2c_client->dev, "mxl692_set_frontend\n");
	...
	status = mxl692_set_demod(dev, demod_type);
	if (status) {
	...
	mpeg_params.mpeg_parallel = 1;
	mpeg_params.msb_first = MXL_EAGLE_DATA_SERIAL_MSB_1ST;
	mpeg_params.mpeg_sync_pulse_width = MXL_EAGLE_DATA_SYNC_WIDTH_BYTE;
	mpeg_params.mpeg_valid_pol = MXL_EAGLE_CLOCK_ACTIVE_HIGH;
	mpeg_params.mpeg_sync_pol = MXL_EAGLE_CLOCK_ACTIVE_HIGH;
	mpeg_params.mpeg_clk_pol = MXL_EAGLE_CLOCK_ACTIVE_LOW;
	mpeg_params.mpeg3wire_mode_enable = 1;
	mpeg_params.mpeg_clk_freq = MXL_EAGLE_MPEG_CLOCK_13_5MHZ;
	mpeg_params.mpeg_pad_drv.pad_drv_mpeg_clk = MXL_EAGLE_IO_MUX_DRIVE_MODE_4X;
	mpeg_params.mpeg_pad_drv.pad_drv_mpeg_dat = MXL_EAGLE_IO_MUX_DRIVE_MODE_4X;
	mpeg_params.mpeg_pad_drv.pad_drv_mpeg_syn = MXL_EAGLE_IO_MUX_DRIVE_MODE_4X;
	mpeg_params.mpeg_pad_drv.pad_drv_mpeg_val = MXL_EAGLE_IO_MUX_DRIVE_MODE_4X;
	status = mxl692_i2c_writeread(dev,
						...
						0);
	if (status) {
		dev_err(&dev->i2c_client->dev, "mxl692_set_frontend MXL_EAGLE_OPCODE_DEVICE_MPEG_OUT_PARAMS_SET err %d\n", status);
		goto err;
	}
	...
	status = mxl692_i2c_writeread(dev,
				      ...
				      0);
	if (status) {
		dev_err(&dev->i2c_client->dev, "mxl692_set_frontend MXL_EAGLE_OPCODE_TUNER_CHANNEL_TUNE_SET err %d\n", status);
		goto err;
	}
	...
	case MXL_EAGLE_DEMOD_TYPE_ATSC:
		status = mxl692_i2c_writeread(dev,
					      ...
					      NULL, 0, NULL, 0);
		if (status) {
			dev_err(&dev->i2c_client->dev, "mxl692_set_frontend MXL_EAGLE_OPCODE_ATSC_INIT_SET err %d\n", status);
			goto err;
		}
		...
		status = mxl692_i2c_writeread(dev,
					      ...
					      NULL, 0, NULL, 0);
		if (status) {
			dev_err(&dev->i2c_client->dev, "mxl692_set_frontend MXL_EAGLE_OPCODE_ATSC_ACQUIRE_CARRIER_SET err %d\n", status);
			goto err;
		}
	...
	case MXL_EAGLE_DEMOD_TYPE_QAM:
		...
		status = mxl692_i2c_writeread(dev,
					      ...
					      NULL, 0);
		if (status) {
			dev_err(&dev->i2c_client->dev, "mxl692_set_frontend MXL_EAGLE_OPCODE_QAM_PARAMS_SET err %d\n", status);
			goto err;
		}
		...
		status = mxl692_i2c_writeread(dev,
					      ...
					      NULL, 0, NULL, 0);
		if (status) {
			dev_err(&dev->i2c_client->dev, "mxl692_set_frontend MXL_EAGLE_OPCODE_QAM_RESTART_SET err %d\n", status);
			goto err;
		}
...
static int mxl692_read_snr(struct dvb_frontend *fe, u16 *snr)
{
	...
	*snr = 0;
	...
	dev_dbg(&dev->i2c_client->dev, "mxl692_read_snr\n");
...
static int mxl692_read_ber_ucb(struct dvb_frontend *fe)
{
	...
	dev_dbg(&dev->i2c_client->dev, "mxl692_read_ber_ucb\n");
			      ...
			      enum fe_status *status)
{
	...
	*status = 0;
	...
	dev_dbg(&dev->i2c_client->dev, "mxl692_read_status\n");
	...
	case MXL_EAGLE_DEMOD_TYPE_ATSC:
		mxl_status = mxl692_i2c_writeread(dev,
						  ...
						  sizeof(struct MXL_EAGLE_ATSC_DEMOD_STATUS_T));
		if (mxl_status)
			dev_dbg(&dev->i2c_client->dev, "mxl692_read_status err: %d\n", mxl_status);
	...
	if (mxl_status)
		dev_err(&dev->i2c_client->dev, "mxl692_read_status err %d\n", mxl_status);
	else
